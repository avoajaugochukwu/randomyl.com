...
import { getHub, getVariant, type PosPreset } from '@/app/config/toolVariants';

const HUB = 'random-adjective-generator';
...
export default async function PosVariantPage({ params }: { params: Promise<{ variant: string }> }) {
  const { variant } = await params;
  ...
  return (
    <>
      ...
      <section className="wrap" id="app">
        <PartOfSpeechGenerator initial={(v.preset as PosPreset).initial ?? 'adjective'} />
      </section>
