...
import { dilemmas, categoryLabels, WyrCategory } from './dilemmas';
...
const categoryKeys = Object.keys(categoryLabels) as WyrCategory[];

export default function WouldYouRatherGenerator() {
  const [category, setCategory] = useState<'all' | WyrCategory>('all');
  ...
  const generate = () => {
    const pool =
      category === 'all'
        ? categoryKeys.flatMap((k) => dilemmas[k])
        : dilemmas[category];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <select id="category" value={category}
            onChange={(e) => setCategory(e.target.value as 'all' | WyrCategory)}
