...
import { questions, categoryLabels, QuestionCategory } from './questions';
...
const categoryKeys = Object.keys(categoryLabels) as QuestionCategory[];
...
export default function QuestionGenerator() {
  ...
  const generate = () => {
    const pool =
      category === 'all'
        ? categoryKeys.flatMap((k) => questions[k])
        : questions[category];
  ...
  return (
    <div className="panel flex flex-col gap-5 max-w-2xl mx-auto w-full p-6 md:p-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <select id="category" value={category}
            onChange={(e) => setCategory(e.target.value as 'all' | QuestionCategory)}
      ...
      <Button onClick={generate} className="w-full">
        <Shuffle className="mr-2 h-4 w-4" /> Generate Questions
      </Button>

      {results.length > 0 && (
        <div className="mt-1">
          <div className="flex justify-between items-center mb-2">
            <p className="text-sm font-medium">{results.length} question{results.length > 1 ? 's' : ''}</p>
