...
import { Copy, CopyCheck, Lightbulb } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { toast } from 'sonner';
import { facts, categoryLabels, type FactCategory } from './facts';
...
const categoryKeys = Object.keys(categoryLabels) as FactCategory[];

export default function FactGenerator() {
  const [category, setCategory] = useState<'all' | FactCategory>('all');
  ...
  const generate = () => {
    const pool =
      category === 'all'
        ? categoryKeys.flatMap((k) => facts[k])
        : facts[category];
  ...
  return (
    <div className="panel flex flex-col gap-5 max-w-2xl mx-auto w-full p-6 md:p-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <select id="category" value={category}
            onChange={(e) => setCategory(e.target.value as 'all' | FactCategory)}
      ...
      <Button onClick={generate} className="w-full">
        <Lightbulb className="mr-2 h-4 w-4" /> Generate Facts
      </Button>

      {results.length > 0 && (
        <div className="mt-1">
          <div className="flex justify-between items-center mb-2">
            <p className="text-sm font-medium">{results.length} fact{results.length > 1 ? 's' : ''}</p>
