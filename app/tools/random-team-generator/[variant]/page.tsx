...
import { getHub, getVariant, type TeamPreset } from '@/app/config/toolVariants';

const HUB = 'random-team-generator';
...
export default async function TeamVariantPage({ params }: { params: Promise<{ variant: string }> }) {
  const { variant } = await params;
  ...
  return (
    <>
      ...
      <section className="wrap" id="app">
        <TeamGenerator preset={v.preset as TeamPreset} />
      </section>
